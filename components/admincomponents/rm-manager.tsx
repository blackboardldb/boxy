      ...
      <div className="px-4 py-6 space-y-6 max-w-2xl mx-auto">
        ...
        <Tabs defaultValue="calculator" className="space-y-6">
          <TabsList className="grid w-full grid-cols-2 h-auto p-1 bg-slate-100">
            <TabsTrigger
              value="calculator"
              className="flex items-center gap-2 py-3 text-slate-600 data-[state=active]:text-zinc-900 data-[state=active]:bg-white"
            >
          ...
          <TabsContent value="calculator" className="space-y-6">
            ...
            <Card className="bg-white border-0">
              <CardHeader>
                <h1 className="flex items-center gap-2 text-zinc-900 text-xl">
                ...
                <div className="mt-4">
                  <Select value={selectedExercise} onValueChange={setSelectedExercise}>
                    <SelectTrigger className="w-full bg-slate-100 text-zinc-900 border-0">
                  ...
                  <div className="mt-4 bg-white rounded-xl p-4">
                    <div className="flex items-center justify-between cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}>
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <span className="font-bold text-sm underline text-zinc-900">Editar</span>
                      </div>
                    </div>
                    ...
                    {isExpanded && (
                      <div className="mt-4 pt-4 border-t border-slate-200">
                        {history.length === 0 ? (
                          <div className="text-center text-slate-600 py-4 text-sm mb-4">
                            <p>Agrega tu primer registro</p>
                          </div>
                        ) : (
                          <div className="grid grid-cols-3 gap-2 text-sm mb-6">
                            <div className="flex flex-col">
                              <span className="text-slate-500 mb-1">Primer registro</span>
                              <span className="text-zinc-900 font-medium">{history[0].weight} {history[0].unit}</span>
                            ...
                            {history.length >= 2 && (
                              <div className="flex flex-col">
                                <span className="text-slate-500 mb-1">{history.length === 2 ? 'Último' : 'Penúltimo'}</span>
                                <span className="text-zinc-900 font-medium">{history[1].weight} {history[1].unit}</span>
                                <span className="text-slate-500 text-xs">
                            ...
                            {history.length === 3 && (
                              <div className="flex flex-col">
                                <span className="text-slate-500 mb-1">Último</span>
                                <span className="text-zinc-900 font-medium">{history[2].weight} {history[2].unit}</span>
                                <span className="text-slate-500 text-xs">
                        ...
                        <div className="mb-6 flex flex-col gap-2">
                          ...
                          <div className="flex items-center gap-2">
                            <Input
                              ...
                              className="bg-slate-100 border-slate-200 text-zinc-900 placeholder:text-slate-500"
                              min="0"
                              step="0.5"
                              placeholder="0"
                            ...
                            >
                              <SelectTrigger className="w-24 bg-slate-100 border-slate-200 text-zinc-900">
                                <SelectValue />
                              </SelectTrigger>
                              <SelectContent>
                        ...
                        <div className="flex gap-2">
                          <Button
                            className="flex-1 bg-zinc-900 text-alumno-bg hover:opacity-80 disabled:opacity-50"
                            onClick={saveRM}
                          ...
                          <Button
                            variant="outline"
                            className="flex-1 bg-transparent border-slate-200 text-zinc-900 hover:bg-slate-100"
              ...
              <CardContent>
                <div className="flex items-center gap-2">
                  ...
                  <Input
                    ...
                    className="text-center text-3xl font-bold flex-1 p-3 appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none text-zinc-900 bg-white border-slate-200"
                    min="0"
                    step="0.5"
            ...
            {calculatorValue > 0 && (
              <Card className="bg-white">
                <CardHeader>
                  <p className="text-lg font-bold text-zinc-900">Tabla de Porcentajes</p>
                </CardHeader>
                <CardContent>
                    ...
                    {percentages.map((percentage) => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className="text-lg font-bold w-12 text-center text-zinc-900">
                            {percentage}%
                          </div>
                          {percentage === 100 && (
