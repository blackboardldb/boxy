...
interface AdminWeeklyDatePickerProps {
  selectedDate: Date;
  onDateSelect: (date: Date) => void;
  className?: string;
}

export default function AdminWeeklyDatePicker({
  selectedDate,
  onDateSelect,
  className = "",
}: AdminWeeklyDatePickerProps) {
  ...
  const getWeekDates = (weekStart: Date): Date[] => {
    if (!weekStart) {
      console.warn("getWeekDates: weekStart inválido", weekStart);
      return [];
    }
    const dates = [];
    for (let i = 0; i < 7; i++) {
