...
- [x] ~~**Bug residual: `endOfDayChile` fija el corte a `T23:59:59.999Z` (UTC), no a medianoche en Chile**~~
  - ~~**Qué falta:** Se verificó que `endOfDayChile` en `lib/utils.ts` usa internamente `fromZonedTime(..., "America/Santiago")` para convertir correctamente las 23:59:59 locales a su instante UTC equivalente. Por tanto, no existe tal bug; el sistema corta a medianoche exacta en Chile. Ticket cerrado.~~
