...
export default function StudentEditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { users, fetchUsers, updateUserById, classSessions, disciplines, fetchClassSessions, fetchDisciplines } = useBlackSheepStore();
  ...
  // Cargar clases y disciplinas si no hay
  useEffect(() => {
  ...
  }, [classSessions, disciplines, fetchClassSessions, fetchDisciplines]);
              ...
              <Tabs defaultValue="proximas">
                ...
                <TabsContent value="proximas" className="mt-4 space-y-4">
                  <div className="flex items-center gap-3 text-sm text-muted-foreground bg-blue-50/50 p-4 border rounded-md">
                    ...
                    {studentUpcomingClasses.length > 0
                      ? `${studentUpcomingClasses.length} clase(s) próxima(s) reservada(s).`
                      : "No tiene clases futuras reservadas."}
                  </div>
                  {studentUpcomingClasses.length > 0 && (
                      ...
                      {studentUpcomingClasses.map((session) => {
                        ...
                        return (
                          <div key={session.id} className="flex justify-between items-center p-3 border rounded-md">
                            <div>
                              <p className="font-medium text-sm">{disc?.name || session.name}</p>
                              <p className="text-xs text-muted-foreground">
                                {sessionDate.toLocaleDateString("es-CL", { weekday: "long", day: "numeric", month: "short" })} • {sessionDate.toLocaleTimeString("es-CL", { hour: "2-digit", minute: "2-digit" })} hrs
                              </p>
                            </div>
                          </div>
                        );
                ...
                <TabsContent value="consumidas" className="mt-4 space-y-4">
                  <div className="flex items-center gap-3 text-sm text-muted-foreground bg-green-50/50 p-4 border rounded-md">
                    ...
                    Este mes ha consumido {student.membership?.centerStats?.currentMonth?.classesAttended || 0} de sus {selectedPlan?.classLimit || 0} clases. Historial de asistencia:
                  </div>
                  {studentPastClasses.length > 0 ? (
                    <div className="space-y-3 opacity-80">
                      {studentPastClasses.map((session) => {
                        const disc = disciplines?.find((d) => d.id === session.disciplineId);
                        const sessionDate = new Date(session.dateTime);
                        return (
                          <div key={session.id} className="flex justify-between items-center p-3 border rounded-md bg-zinc-50">
                            <div>
                              <p className="font-medium text-sm line-through">{disc?.name || session.name}</p>
                              <p className="text-xs text-muted-foreground">
                                {sessionDate.toLocaleDateString("es-CL", { weekday: "long", day: "numeric", month: "short" })} • Asistió
                              </p>
                            </div>
                          </div>
                        );
                    ...
                    <p className="text-sm text-muted-foreground text-center py-4">Sin historial de clases asistidas.</p>
                  )}
                </TabsContent>
